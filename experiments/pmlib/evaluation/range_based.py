...
class _TSMetric:
    """ Title: Time-series-precision-recall
    Author: Alexey Kurochkin
    Date: 2020-05-28
    Link: https://github.com/KurochkinAlexey/Time-series-precision-recall """

    def __init__(self, metric_option="classic", beta=1.0, alpha_r=0.0,
                 cardinality="one", bias_p="flat", bias_r="flat"):
    ...
    def _delta_function(self, t, anomaly_length, task_type):
        if task_type == 0:
            return self._delta_select(
                self.bias_p, t, anomaly_length, task_type)
        elif task_type == 1:
            return self._delta_select(
                self.bias_r, t, anomaly_length, task_type)
        else:
    ...
    def _update_precision(self, real_anomalies, predicted_anomalies):
        ...
        for i in range(len(predicted_anomalies)):
            ...
            overlap_reward = self._gamma_function(
                overlap_count, 0)*omega_reward
            if overlap_count[0] > 0:
                existence_reward = 1
            else:
                existence_reward = 0

            precision += self.alpha_p*existence_reward + \
                (1 - self.alpha_p)*overlap_reward
        precision /= len(predicted_anomalies)
        return precision

    def _calculate_tp(self, real_anomalies, predicted_anomalies):
        tp = 0
        ...
        for i in range(len(real_anomalies)):
            ...
            tp += self.alpha_r*existence_reward + \
    ...
    def _update_recall(self, real_anomalies, predicted_anomalies):
        ...
        for i in range(len(real_anomalies)):
            ...
            recall += self.alpha_r*existence_reward + \
                (1 - self.alpha_r)*overlap_reward
...
#         assert np.allclose(np.unique(values_pred), np.array([0, 1]))
        ...
        if self.metric_option == "classic":
            ...
            predicted_anomalies = np.argwhere(
                values_pred == 1).repeat(2, axis=1)

        elif self.metric_option == "time-series":
            predicted_anomalies_ = np.argwhere(values_pred == 1).ravel()
            if predicted_anomalies_.shape[0] == 0:
                predicted_anomalies = []
            else:
                predicted_anomalies_shift_forward = self._shift(
                    predicted_anomalies_,
                    1,
                    fill_value=predicted_anomalies_[0])
                ...
                predicted_anomalies_start = np.argwhere(
                    (
                        predicted_anomalies_shift_forward -
                        predicted_anomalies_
                    ) != -1).ravel()
                predicted_anomalies_finish = np.argwhere(
                    (
                        predicted_anomalies_ -
                        predicted_anomalies_shift_backward
                    ) != -1).ravel()
                predicted_anomalies = np.hstack([
                    ...
                    (
                        predicted_anomalies_[predicted_anomalies_finish]
                    ).reshape(-1, 1)]
                )

            real_anomalies_ = np.argwhere(values_real == 1).ravel()
            if real_anomalies_.shape[0] == 0:
                real_anomalies = []
            else:
    ...
    def precision(self, values_real, values_predicted):
        # if np.max(values_real) == 0 or np.max(values_predicted) == 0:
        #     return 0
        # if np.min(values_real) == 1 or np.min(values_predicted) == 1:
        #     return 1
        real_anomalies, predicted_anomalies = self._prepare_data(
            values_real, values_predicted)
        precision = self._update_precision(real_anomalies, predicted_anomalies)
    ...
    def recall(self, values_real, values_predicted):
        # if np.sum(values_real) == 0 or np.sum(values_predicted) == 0:
        #     return 0
        real_anomalies, predicted_anomalies = self._prepare_data(
            values_real, values_predicted)
        recall = self._update_recall(real_anomalies, predicted_anomalies)
    ...
    def tp(self, values_real, values_predicted):
        # if np.sum(values_real) == 0 or np.sum(values_predicted) == 0:
        #     return 0
        real_anomalies, predicted_anomalies = self._prepare_data(
            values_real, values_predicted)
        tp = self._calculate_tp(real_anomalies, predicted_anomalies)
        return tp

    def f_beta(self, values_real, values_predicted):
        # if np.sum(values_real) == 0 or np.sum(values_predicted) == 0:
        #     return 0
        real_anomalies, predicted_anomalies = self._prepare_data(
            values_real, values_predicted)
        ...
        if precision + recall != 0:
            Fbeta = (
                (1 + self.beta**2)
                * precision*recall/(self.beta**2*precision + recall)
            )
        else:
            Fbeta = 0
        return Fbeta
...
def precision_ts(y_true, y_pred, bias, cardinality):
    metric = _TSMetric(metric_option="time-series",
                       cardinality=cardinality,
                       bias_p=bias)
    return metric.precision(y_true, y_pred)


def recall_ts(y_true, y_pred, alpha, bias, cardinality):
    metric = _TSMetric(metric_option="time-series",
                       alpha_r=alpha,
                       cardinality=cardinality,
                       bias_r=bias)
    return metric.recall(y_true, y_pred)


def f_beta_ts(y_true, y_pred, alpha_r, bias_p, bias_r, beta, cardinality):
    metric = _TSMetric(metric_option="time-series",
                       beta=beta,
                       alpha_r=alpha_r,
                       cardinality=cardinality,
                       bias_p=bias_p,
                       bias_r=bias_r)
    return metric.f_beta(y_true, y_pred)
